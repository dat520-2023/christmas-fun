...
func testGRPCCalls(tests []gRPCTest, testGroupName string, t *testing.T) {
	for _, test := range tests {
		...
		t.Run(test.desc, func(t *testing.T) {
			for _, action := range test.setup {
				switch {
				case action.command == INSERT:
					performInsert(action, server, t)
				case action.command == LOOKUP:
					performLookup(action, server, t)
				case action.command == KEYS:
					performKeys(action, server, t)
				}
			}
...
//******************************************************************************
// This function calls the Insert() function on the server.
// It updates the score if this is not a setup command.
// Input: action     A pointer to the gRPCAction to perform
//        server     A pointer to the keyValueServicesServer
//        t          A pointer to the testing struct
//******************************************************************************
func performInsert(action gRPCAction, server *keyValueServicesServer, t *testing.T) {
	...
	if !cmp.Equal(got, action.want) {
		t.Errorf("Want: %v, Got: %v", action.want, got)
	}
}

//******************************************************************************
// This function calls the Lookup() function on the server.
// It updates the score if this is not a setup command.
// Input: action     A pointer to the gRPCAction to perform
//        server     A pointer to the keyValueServicesServer
//        t          A pointer to the testing struct
//******************************************************************************
func performLookup(action gRPCAction, server *keyValueServicesServer, t *testing.T) {
...
//******************************************************************************
// This function calls the Keys() function on the server.
// It updates the score if this is not a setup command.
// Input: action     A pointer to the gRPCAction to perform
//        server     A pointer to the keyValueServicesServer
//        t          A pointer to the testing struct
//******************************************************************************
func performKeys(action gRPCAction, server *keyValueServicesServer, t *testing.T) {
	...
	if !cmp.Equal(len(got), len(want)) {
		t.Fatalf("Want: %v, Got: %v", want, got)
	}

	// Check that the keys match what is expected
	for index := range got {
		if !cmp.Equal(got[index], want[index]) {
