...
func TestEchoServerProtocol(t *testing.T) {
	...
	if err != nil {
		t.Errorf("TestEchoServer: got error when setting up server: %v", err)
	}

	go server.ServeUDP()

	if err := waitForServer(addr); err != nil {
		t.Fatalf("TestEchoServer: failed to connect to the server: %v", err)
	}
...
func TestPreAndPostSetup(t *testing.T) {
	...
	if err == nil {
		t.Errorf("TestPreAndPostSetup: want error for SendCommand pre server setup, got none")
	}
	if !(strings.Contains(err.Error(), "connection refused") || strings.Contains(err.Error(), "WSARecv")) {
		t.Errorf("TestPrePostSetup: got error pre server setup, but want connection refused or WSARecv error")
	}
	server, err := NewUDPServer(addr)
	if err != nil {
		t.Errorf("TestPrePostSetup: got error when setting up server: %v", err)
	}

	go server.ServeUDP()

	if err := waitForServer(addr); err != nil {
		t.Fatalf("TestPrePostSetup: failed to connect to the server: %v", err)
	}

	_, err = SendCommand(addr, "UPPER", "FooBar")
	if err != nil {
		t.Errorf("TestPrePostSetup: want no error for SendCommand post setup, got %v", err)
	}
}
...
func TestMalformedRequest(t *testing.T) {
	...
	server, err := NewUDPServer(addr)
	if err != nil {
		t.Errorf("TestMalformedRequest: got error when setting up server: %v", err)
	}

	go server.ServeUDP()

	if err := waitForServer(addr); err != nil {
		t.Fatalf("TestMalformedRequest: failed to connect to the server: %v", err)
	}
